...
femm_opt = struct('deg', 2, 'qdeg', 6, 'min_area', 1e-2, 'edge', nodes);
opt = struct('femm_opt', femm_opt, 'omega', 20, 'kappa', 2.0);

obj = otpat(opt);
[obj.measurement, ~, ~, ~, ~] = obj.forward(obj.parameter, 0.0);

%%
tic;
obj.reg = struct('d' , 1e-2, 'a', 1e-4, 'g', 1e-6);
